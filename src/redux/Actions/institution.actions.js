...
const register = institution => {
  return dispatch => {
    dispatch(
      actionHelper.request(institutionConstants.INSTITUTION_REGSITER_REQUEST)
    );
        ...
        if (actionHelper.successCheck(response)) {
          dispatch(
            actionHelper.success(
              institutionConstants.INSTITUTION_REGSITER_SUCCESS,
              institution
            )
          );
          ...
          dispatch(
            actionHelper.failure(
              institutionConstants.INSTITUTION_REGSITER_FAILURE,
              response.data.error
            )
          );
          dispatch(
...
const getAll = condition => {
  return dispatch => {
    dispatch(
      actionHelper.request(institutionConstants.INSTITUTION_GETALL_REQUEST)
    );
    institutionService.getAll(condition).then(
      response => {
        if (actionHelper.successCheck(response)) {
          dispatch(
            actionHelper.success(
              institutionConstants.INSTITUTION_GETALL_SUCCESS,
              response.data.data
            )
          );
...
const _delete = id => {
  return dispatch => {
    dispatch(
      actionHelper.request(institutionConstants.INSTITUTION_DELETE_REQUEST,{id})
    );
    institutionService.delete(id).then(
      response => {
        if (actionHelper.successCheck(response)) {
          dispatch(
            actionHelper.success(
              institutionConstants.INSTITUTION_DELETE_SUCCESS,
              { id }
            )
          );
          ...
          dispatch(
            actionHelper.failure(
              institutionConstants.INSTITUTION_DELETE_FAILURE,
