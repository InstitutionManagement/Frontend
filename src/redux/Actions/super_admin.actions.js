...
function deleteSuperAdmin(authId, superAdminId) {
  return dispatch => {
    dispatch(actionHelper.request(superAdminConstants.SUPERADMIN_DELETE_REQUEST, { superAdminId }));
    superAdminService.deleteSuperAdmin(authId, superAdminId).then(
      response => {
        if (response.status === 200 && response.data.error === null) {
          dispatch(actionHelper.success(superAdminConstants.SUPERADMIN_DELETE_SUCCESS, { superAdminId }));
